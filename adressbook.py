...
class AddressBook(UserDict):
    ...
    def view(self):
        ...
        for key, el in self.data.items():
            ...
            print("{:<18} {:<10}".format(
                "Phone number(s):", ", ".join(el["Number"])))
            print("{:<18} {:<10}".format(
                "Email address(es):", ", ".join(el["Email"])))
    ...
    def add_phone(self):
        ...
        if contact_enter in self.data.keys():
            ...
            if re.match(Contact._PATTERN_FOR_PHONE, added):
                ...
                print(
                    f"Additional phonenumber to contact {contact_enter} was successfully added")
            else:
                print("Invalid phone number. Try again next time!")
        else:
            print(
                f"Contact {contact_enter} isn't exist. Select one of your existing contacts!")
    ...
    def add_email(self):
        ...
        if contact_enter in self.data.keys():
            ...
            if re.match(Contact._PATTERN_FOR_EMAIL, added):
                ...
                print(
                    f"Backup email address to contact {contact_enter} was successfully added")
            else:
                print("Invalid email address. should be 'examples@com.ua'")
        else:
            print(
                f"Contact {contact_enter} isn't exist. Check the correctness of the data and try again!")
    ...
    def coming_birthdays(self):
        ...
        bd_actual = dict()
        period = int(input(
            "Enter the end period (number of days) to search for upcoming celebrations: "))
        ...
        for contact_name, contact_data in self.data.items():
            for key, value in contact_data.items():
                if key == "Birthdate" and value:
                    current_bd.update(
                        {contact_name: value.replace(year=datetime.now().year)})
        for name, date in current_bd.items():
            if date < start_period:
                bd_actual.update(
                    {name: date.replace(year=start_period.year + 1)})
            else:
                bd_actual.update({name: date})
        bd_dict = {name: date for name, date in bd_actual.items(
        ) if end_period >= date >= start_period}
    ...
    def find(self):
        search_str = str(
            input("Enter name or phonenumber for search (include partly): ")).lower()
        print("!" * 40)
        contact_find = False
        for name, phone in self.data.items():
            ...
            if search_str in name_lower or search_str in str([phone["Number"]]):
                ...
                print("{:<18} {:<10}".format(
                    "Phone number(s):", ", ".join(phone["Number"])))
                print("{:<18} {:<10}".format(
                    "Email address(es):", ", ".join(phone["Email"])))
    ...
    def remove_contact(self):
        ...
        if index in self.data:
            self.data.pop(index)
            print(
                f"Contact {index} was successfully removed from your addressbook")
        else:
            ...
            if enter == name:
                ...
                if attribute.lower() in ["name", "person", "subject"]:
                    ...
                    if re.match(Contact._PATTERN_FOR_NAME, new_value) and new_value != name:
                        ...
                        print(
                            f"Contact name {name} was successfully edited to {new_value}")
                    else:
                        print(
                            "Invalid name. Should starts with upper case and consists real name")
                    ...
                    if re.match(Contact._PATTERN_FOR_EMAIL, new_value):
                        ...
                        print(
                            f"{name}'s email address was successfully edited to {new_value}")
                    else:
                        print("Invalid email address. should be 'examples@com.ua'")
                    ...
                    if re.match(Contact._PATTERN_FOR_PHONE, new_value):
                        ...
                        print(
                            f"{name}'s phone number was successfully edited to {new_value}")
                    else:
                    ...
                    if new_value:
                        ...
                        true_value = datetime.strptime(
                            new_value, "%d.%m.%Y").date()
                        record["Birthdate"] = true_value
                        print(
                            f"{name}'s birthdate  was successfully edited to {true_value}")
                    else:
                    ...
                    if re.match(Contact._PATTERN_FOR_ADRESS, new_value):
                        ...
                        print(
                            f"{name}'s street address was successfully edited to {new_value}")
                    else:
    ...
    def load(self):
        global was_saved
        file_name = input(
            "Write name of file that should be created in format 'name.file_extension': ")
    ...
    def ab_cont(self):
        while True:
            file_name = input(
                "Write name of file from where should be imported data in format 'name.file extension': ")
            if re.match(r"[\w\d]+\.\w+", file_name):
                try:
...
class Contact(UserDict):
    ...
    """
    Цей клас, що формує контактну книгу (всю інформацію про один певний контакт)
    """
    ...
    def email(self, value):
        if value:
            if re.match(self._PATTERN_FOR_EMAIL, value):
                ...
                print(
                    "Wrong email address: Format: 'name@example.com' or 'name@example.com.ua' ")
    ...
    def birthdate(self, value):
        try:
            if value:
                ...
                self.__birthdate = datetime.strptime(
                    value, "%d.%m.%Y").date()  # Валідація дати народження
        except ValueError:
            print(
                "Invalid birtdate. Expecting 'DD.MM.YYYY', with punctuation (' ,-/'")


def write_contact():
    """
    Допоміжна  команда, яка створює контактну картку й одразу проходить всі валідації. Якщо все проходить валідацію - записується  в адрессбук
    """
    global contact
    ...
    while True:
        address = input(
            "Address (for example, 'Mykhaylo Kandinsky St. 23/422'):  ")
    ...
    while True:
        email = input(
            "Email (for example, 'name@example.com' or 'name@example.com.ua'): ")
...
commands = {
    # Команда, яка формує контактну картку і записує її одразу в адрессбук
    write_contact: "record",
    ab.view: "view",  # Перегляд одразу всього адрессбуку
    # Команда, яка додає номер до існуючого контакту, як додатковий
    ab.add_phone: "add number",
    info_command: "commands",  # Виводимо список всіх доступних команд
    ab.add_email: "add email",  # Додає email до існуючого контакту
    ab.coming_birthdays: "upcoming",
    # Команда, яка виводить дні народження записаних контактів в періоді від сьогодні до заданої кількості днів
    ab.find: "find",  # Команда, яка знаходить контактів за номером телефону або ім'ям
    ab.remove_contact: "remove",  # Видаляється  зазначений контакт
    ab.edit_contact: "edit",  # Редагуються атрибути зазначеного контакту
    ab.load: "save",  # Зберігається файл
    ab.ab_cont: "open"  # Відкриває файл
}
...
def start():  # точка входу
    ...
    while True:
        ...
        if command in commands.values():
            ...
            print(
                "Wrong command! Choose one of available commands!\n(Review available commands - 'commands')")
